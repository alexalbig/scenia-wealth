...
| C11 | **"Situación actual" (plan base) no cuenta como escenario, tenga eventos o no** | `AHORA` — **corrección pendiente** | Se implementó como "cuenta si tiene eventos", y García-Llorente muestra **6**. Debe mostrar **5**. Las dos jubilaciones del plan base vienen del seed, no las montó nadie — y en un cliente real el plan base tendría eventos desde el primer día, porque la jubilación prevista es un dato del expediente. La columna debe significar **"alternativas montadas"**: cuánto ha explorado el asesor. El criterio de "solo con eventos" sí se mantiene para las alternativas |
...
| PE12 | **La columna "Ingresos del año" dice 0 € cuando el estado dice "sin ingresos"** | `AHORA` | Dos mensajes contradictorios en la misma fila: **el estado declara el hueco y la columna afirma un cero**. Debe decir «—» o «sin informar». Lo mismo con «Patrimonio atribuido» si la persona no tiene titularidades |
| PE4 | **Añadir jubilación prevista a la lista** | `SIGUIENTE` | Sí está en la ficha F1, bien resuelta y marcada como *estimación del asesor*. Lo que falta es verla en la vista de conjunto, que es donde el asesor la busca al hablar de rescatar un plan o vender la vivienda |
...
| PE5 | **Las cotizaciones a la Seguridad Social son un dato de la persona, no del ingreso** | `SIGUIENTE` | Hoy se introducen en la pestaña Ingresos. Conceptualmente pertenecen a la persona, y ahí es donde un asesor las buscaría |
...
| F3 | **Confirmado: la frase de lectura solo cuenta un escalón** | `AHORA` — en arreglo | Verificado con Marisa Blasco el 06/08/2026. Su recorrido real es 30 % durante 2.210 € · 32,5 % durante 3.200 € · 36 % después, y la pantalla presenta el 32,5 % como si durase. Con Andreu el segundo escalón dura 28.000 € y por eso la frase se sostenía: **el defecto solo se ve con umbrales cercanos**. Un asesor que planifique 5.000 € a nombre de Marisa lee 32,5 % y paga 36 % en el último tramo. Se arregla junto con F5 y F6 |
| F4 | **El margen puede depender de un dato tecleado a mano con precisión de decenas de euros** | `AHORA` | Caso Andreu Ferrer: base 71.950 € contra umbral autonómico 72.000 €. Un error de 50 € en las cotizaciones cambia el marginal del 47,5 % al 49 %. **Cuando el margen sea inferior a ~500 €, la pantalla debe advertir de la sensibilidad**, no dar la cifra como firme. Enlaza con M7 |
| F5 | **Confirmado en pantalla: el margen de 50 € se presenta como si fuera recorrido** | `AHORA` — en arreglo | Verificado con Andreu Ferrer el 06/08/2026. La cifra sale en tipografía grande y la frase de lectura dice "hasta agotar 50 €": las dos son literalmente ciertas y se leen como si quedara margen. **Debe comunicar "sin margen"**. Se resuelve junto con F6 y F3 en una sola intervención, no por separado |
| F6 | **Los dos márgenes compiten con la misma jerarquía visual, y solo uno decide** | `AHORA` — en arreglo | Andreu muestra 50 € (autonómico) y 228.050 € (estatal) con el mismo tamaño, peso y frase. **El margen real es siempre el menor de los dos**, porque es el primero que se agota. El estatal por encima de 60.000 € nunca informa una decisión de un cliente de EAF: nadie genera 228.000 € de renta adicional. La pantalla ya sabe la respuesta —la frase de lectura la da bien—, lo que falta es que las dos columnas no compitan con ella |
